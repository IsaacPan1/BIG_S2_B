...
# ── Walk-forward split (panel_forecasting) ─────────────────────────────────
...
# ── Optuna tuning (10 trials, 10-minute timeout) ──────────────────────────
...
TUNING_DEADLINE = start_time + 10 * 60  # 10 minutes
...
def objective(trial):
    ...
    params = {
        ...
        "num_leaves": trial.suggest_int("num_leaves", 4, 31),  # small — only 5 patients
...
try:
    ...
    study.optimize(objective, n_trials=10, timeout=10*60, catch=(Exception,))
...
probe_params = {
    ...
    "n_estimators": 1000, "bagging_freq": 5,
...
# ── OOF on training data (for validator) ──────────────────────────────────
# For panel_forecasting, use leave-one-patient-out as OOF
final_params = {
    "objective": "regression_l1", "metric": "mae",
    "n_estimators": best_n_est, "bagging_freq": 5,
    "verbose": -1, "n_jobs": -1,
    **final_hparams,
}
...
# ── Final ensemble on all training data ───────────────────────────────────
seed_preds = []
...
# ── Write reports/oof_predictions.csv ────────────────────────────────────
...
with open("reports/modeler_was_here.txt", "w") as f:
    f.write(f"modeler sub-agent executed at {datetime.datetime.utcnow().isoformat()}Z\n")
