...
def _train_catboost_fold(X_tr_df, y_tr, X_va_df, y_va_orig, hparams,
    ...
    seed_preds, models = [], []
    base_params = {
        "iterations":    hparams.get("iterations", 400),
...
TUNING_DEADLINE = start_time + 25 * 60  # 25 min cap on the nested-CV phase

for of_i, (outer_tr_idx, outer_va_idx) in enumerate(outer_splits):
    elapsed = time.time() - start_time
    if elapsed > TUNING_DEADLINE - start_time:
        print(f"  Outer fold {of_i}: tuning budget exceeded — using defaults")
        break
    ...
    _per_fold_timeout = max(60, int((TUNING_DEADLINE - time.time()) / max(1, len(outer_splits) - of_i)))
    study.optimize(objective, n_trials=8, timeout=_per_fold_timeout, catch=(Exception,))
    ...
    # Train final outer-fold model with best params; predict on outer-val
    try:
        preds, _, _ = _train_catboost_fold(
            X_outer_tr_df, y_outer_tr, X_outer_va_df, y_outer_va_orig,
            best_params, sample_weight=sw_outer_tr, seeds=(42, 7, 123))
    except Exception as _e:
